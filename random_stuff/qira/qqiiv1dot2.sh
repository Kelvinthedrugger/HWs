...
if [[ "$unamestr" == 'Linux' ]]; then
  ...
  if [ $(which apt-get) ]; then
    ...
    echo "installing apt packages"
     apt-get -qq -y install build-essential debootstrap debian-archive-keyring libjpeg-dev zlib1g-dev unzip wget graphviz curl
    echo "install python packages"
     apt-get -qq -y install python-dev python-pip python-virtualenv
    ...
    echo "installing pip"
     pacman -S --needed --noconfirm base-devel python2-pip python2-virtualenv
    PIP="pip2"
  elif [ $(which dnf) ]; then
     dnf install -y python-pip python-devel gcc gcc-c++ python-virtualenv glib2-devel
    PIP="pip2"
  elif [ $(which yum) ]; then
     yum install -y python-pip python-devel gcc gcc-c++ python-virtualenv glib2-devel
  fi
